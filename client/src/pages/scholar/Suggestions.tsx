...
export default function Suggestions() {
      ...
      try {
        ...
        setSuggestions(
          Array.isArray(response?.data?.suggestions)
            ? (response.data.suggestions as SuggestionEntry[])
            : [],
        );
      } catch (error) {
        toast({
  ...
  const filteredSuggestions = useMemo(() => {
    ...
    return suggestions.filter((entry) =>
      `${entry.subject} ${entry.note || ""}`.toLowerCase().includes(term),
    );
  }, [search, suggestions]);
  ...
  const handleAdd = async () => {
    ...
    try {
      setBusyEntryId("new");
      const response = await addSuggestion(category, subject.trim(), note.trim() || undefined);
      const createdEntry = response?.data as SuggestionEntry;
      setSuggestions((current) => [createdEntry, ...current]);
      setSubject("");
      setNote("");
      toast({
                          ...
                          <div>
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              >
                                ...
                                {categoryOption?.label ?? "Suggestion"}
                              </Badge>
