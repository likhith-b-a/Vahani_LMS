...
export function AdminOverviewSection({ summary }: AdminOverviewSectionProps) {
  ...
  const overviewProgrammes = useMemo(
    () => summary?.programmes ?? [],
    [summary?.programmes],
  );
  ...
  const overviewStats = useMemo(
    () => [
      ...
      },
      {
        ...
        value: overviewProgrammes.filter((programme) => programme.selfEnrollmentEnabled).length,
    ...
    [announcementCount, openQueryCount, overviewProgrammes, summary?.stats],
  );

  return (
    <div className="space-y-6">
      <section className="overflow-hidden rounded-3xl border border-border bg-gradient-to-br from-vahani-blue/10 via-background to-vahani-gold/10">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
            ...
            <div className="rounded-2xl border border-border bg-card/80 p-4">
              ...
              <p className="text-sm text-muted-foreground">
                {openQueryCount} open support queries
              </p>
            </div>
          </div>
        </div>
